                        ...
                        const EomConfig& cfg)
{
  using namespace std::string_literals;
    // Need at least the name with no options to create default config
  if (tokens.size() < 1) {
     throw std::invalid_argument("eom_app::parse_propagator_config() "s +
  ...
  return std::make_pair(name, propCfg);
}
                             ...
                             eom::PropagatorConfig& pCfg)
{
  ...
  if (prop_toks.size() > 3  &&  prop_toks[0] == "Integrator") {
    prop_toks.pop_front();
    if (prop_toks[0] == "RK4") {
      prop_toks.pop_front();
                            ...
                            eom::PropagatorConfig& pCfg)
{
  ...
  if (sun_toks.size() > 1  &&  sun_toks[0] == "SunGravity") {
    ...
    } else if (sun_toks[0] == "Ephemeris") {
      sun_toks.pop_front();
      pCfg.setSunGravityModel(eom::SunGravityModel::eph);
    }
  }
}
...
static void parse_other_model(std::deque<std::string>& other_toks,
                              eom::PropagatorConfig& pCfg)
{
    // "MoonGravity  Model"
