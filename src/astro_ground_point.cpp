...
// The geodetic latitude method is based on one of two iterative methods
//
...
// or
// Vallado 4th edition Alg 12:  ECF to LatLon
GroundPoint::GroundPoint(const Eigen::Matrix<double, 3, 1>& xyz, bool fukushima)
{
  ...
  double rj {m_xyz(1)};
  double rk {m_xyz(2)};
  double rd {std::sqrt(ri*ri + rj*rj)};
  ...
  if (ri == 0.0  &&  rj == 0.0) {
    ...
    if (fukushima) {
      ...
      double gdlat_old {gdlat};
      itr = -1;
      for (int ii=0; ii<max_itr; ++ii) {
        double sgd {std::sin(gdlat)};
        double ce {phy_const::earth_smaj/
                   std::sqrt(1.0 - phy_const::ecc2*sgd*sgd)};
        gdlat = std::atan((rk + ce*phy_const::ecc2*sgd)/rd);
        itr++;
        if (std::fabs(gdlat - gdlat_old) < tol) {
          itr = ii + 1;
          break;
        }
        gdlat_old = gdlat;
