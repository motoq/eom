...
template<typename T, int N>
Eigen::Matrix<T, N, 1> Hermite2<T,N>::getX(T dt) const
{
  if (dt < 0  ||  dt > m_dt_max) {
    throw std::invalid_argument("Hermite2<T,N>::getX(T dt) - bad dt");
  }
...
template<typename T, int N>
Eigen::Matrix<T, N, 1> Hermite2<T,N>::getdX(T dt) const
{
  if (dt < 0  ||  dt > m_dt_max) {
    throw std::invalid_argument("Hermite2<T,N>::getdX(T dt) - bad dt");
  }
